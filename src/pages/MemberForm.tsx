...
import { membersApi, VMemberCreate, VMemberUpdate } from '../services/membersApi';
import { lookupsApi, Service, Rank, Status, State, District, PaymentMode } from '../services/lookupsApi';
...
export const MemberForm = ({ memberId, onNavigateBack }: MemberFormProps) => {
  ...
  const fetchLookupData = async () => {
    ...
    try {
      // Added payment modes to fetch
      const [servicesData, statusesData, statesData, paymentModesData] = await Promise.all([
        lookupsApi.getServices(token),
        lookupsApi.getStatuses(token),
        lookupsApi.getStates(token),
        lookupsApi.getPaymentModes(token),
      ]);
      ...
      if (paymentModesData.length > 0) {
        setFormData(prev => ({ ...prev, payment_mode_id: paymentModesData[0].id }));
      }
    } catch (err) {
  ...
  const fetchMember = async () => {
    ...
    try {
      ...
      setFormData({
        ...
        // Prefill subscription details if available
        subscription_remitted_date: member.subscription_remitted_date || '',
        payment_mode_id: member.payment_mode_id || 1,
        transaction_id: member.transaction_id || '',
        subscription_end_date: member.subscription_end_date || '',
        subscriber_status: member.subscriber_status ? 'Active' : 'Inactive',
      });
    } catch (err) {
    ...
    try {
      if (isEditMode && memberId) {
        const updateData: VMemberUpdate = {
          ...
          subscription_remitted_date: formData.subscription_remitted_date || undefined,
          payment_mode_id: formData.payment_mode_id || undefined,
          ...
          <div className="p-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="pt-4 border-t border-gray-200">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <Input
                    label="Subscription Remitted Date"
                    type="date"
                    value={formData.subscription_remitted_date}
                    onChange={(e) =>
                      setFormData({ ...formData, subscription_remitted_date: e.target.value })
                    }
                  />
                  ...
                  <div className="w-full">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setFormData({ ...formData, payment_mode_id: Number(e.target.value) })
                      }
                      className="w-full px-4 py-2.5 bg-white border border-gray-300 rounded-lg text-gray-900 transition-all duration-200 outline-none focus:border-teal-500 focus:ring-2 focus:ring-teal-200"
                    >
                  ...
                  {/* Only show Transaction ID when Online is selected */}
                  {(() => {
                    const selectedMode = paymentModes.find(
                      (m) => m.id === formData.payment_mode_id
                    );
                    return selectedMode?.mode_name?.toLowerCase() === 'online' ? (
                      <Input
                  ...
                  {/* Read-only subscription end date */}
                  {isEditMode && (
                    <>
                      <Input
                      ...
                      <Input
                        ...
                        type="text"
                        value={formData.subscriber_status}
                        readOnly
                      />
                    </>
                  )}
